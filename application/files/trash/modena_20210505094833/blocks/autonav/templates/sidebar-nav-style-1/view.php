...
foreach ($navItems as $ni) {
    ...
    if ($ni->isCurrent) {
        ...
        $classes[] = 'sidebar-nav-selected';
    }

    if ($ni->inPath) {
        //class for parent items of the page currently being viewed
        $classes[] = 'sidebar-nav-path-selected';
    }
...
if (count($navItems) > 0) {
    echo '<div class="sidebar-nav-style-1"><ul class="sidebar-nav">'; //opens the top-level menu

    foreach ($navItems as $ni) {

        if($ni->level == '1'){
            echo '<li class="' . $ni->classes . ' sidebar-nav-item">'; //opens a nav item
        } else {
        ...
        if ($ni->hasSubmenu) {
            echo '<ul class="is-sidebar-submenu">'; //opens a dropdown sub-menu
        } else {
