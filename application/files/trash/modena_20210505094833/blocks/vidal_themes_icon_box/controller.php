<?php  namespace Concrete\Package\Modena\Block\VidalThemesIconBox;
...
class Controller extends BlockController
{
    ...
    protected $btTable = 'btVidalThemesIconBox';
    ...
    {
        return t("Create an icon box on your site.");
    }
    ...
    {
        $content = array();
        $content[] = $this->iconBoxContent;
        return implode(" ", $content);
    }

    public function view()
    {
        $this->set('iconBoxContent', LinkAbstractor::translateFrom($this->iconBoxContent));
        $iconBoxBorder_options = array(
            '' => "-- " . t("None") . " --",
            'icon-box--bordered' => t("Display Borders"),
            'icon-box--std' => t("Do Not Display Border")
        );
        $this->set("iconBoxBorder_options", $iconBoxBorder_options);
        if (trim($this->iconBg) == "") {
            $this->set("iconBg", '#ffffff');
        }
        $iconPosition_options = array(
            '' => "-- " . t("None") . " --",
            'icon-box__icons--icons-left' => t("Icon Left"),
            'icon-box__icons--icons-std' => t("Icon Top")
        );
        $this->set("iconPosition_options", $iconPosition_options);
    ...
    public function edit()
    {
        ...
        $this->set('iconBoxContent', LinkAbstractor::translateFromEditMode($this->iconBoxContent));
    }

    protected function addEdit()
    {
        $this->set('app', $this->app);
        $this->set('chooseIcon_options', (!in_array("chooseIcon", $this->btFieldsRequired) ? array("" => "-- " . t("None") . " --") : array()) + $this->fontAwesomeIcons('4.6'));
        $this->set("iconBoxBorder_options", array(
                '' => "-- " . t("None") . " --",
                'icon-box--bordered' => t("Display Borders"),
                'icon-box--std' => t("Do Not Display Border")
            )
        );
        $this->set("iconPosition_options", array(
                '' => "-- " . t("None") . " --",
                'icon-box__icons--icons-left' => t("Icon Left"),
                'icon-box__icons--icons-std' => t("Icon Top")
            )
        ...
        $al->register('css', 'auto-css-vidal_themes_icon_box', 'blocks/vidal_themes_icon_box/auto.css', array(), 'modena');
        $this->requireAsset('redactor');
        $this->requireAsset('core/file-manager');
        $this->requireAsset('core/colorpicker');
        $this->requireAsset('css', 'auto-css-vidal_themes_icon_box');
    ...
    {
        $args['iconBoxContent'] = LinkAbstractor::translateTo($args['iconBoxContent']);
    ...
    public function validate($args)
    {
        ...
        if (trim($args["chooseIcon"]) == "") {
            if (in_array("chooseIcon", $this->btFieldsRequired)) {
                $e->add(t("The %s field is required.", t("Choose Icon")));
            }
        } elseif (!array_key_exists($args["chooseIcon"], $this->fontAwesomeIcons('4.6'))) {
            $e->add(t("The %s field requires a valid Font Awesome icon.", t("Choose Icon")));
        }
        if (in_array("iconBoxContent", $this->btFieldsRequired) && (trim($args["iconBoxContent"]) == "")) {
            $e->add(t("The %s field is required.", t("Icon Box Content")));
        }
        if ((in_array("iconBoxBorder", $this->btFieldsRequired) && (!isset($args["iconBoxBorder"]) || trim($args["iconBoxBorder"]) == "")) || (isset($args["iconBoxBorder"]) && trim($args["iconBoxBorder"]) != "" && !in_array($args["iconBoxBorder"], array("icon-box--bordered", "icon-box--std")))) {
            $e->add(t("The %s field has an invalid value.", t("Icon Box Border")));
        }
        if (in_array("iconColor", $this->btFieldsRequired) && (trim($args["iconColor"]) == "")) {
            $e->add(t("The %s field is required.", t("Icon Color")));
        }
        if (in_array("iconBg", $this->btFieldsRequired) && (trim($args["iconBg"]) == "")) {
            $e->add(t("The %s field is required.", t("Icon Background")));
        }
        if ((in_array("iconPosition", $this->btFieldsRequired) && (!isset($args["iconPosition"]) || trim($args["iconPosition"]) == "")) || (isset($args["iconPosition"]) && trim($args["iconPosition"]) != "" && !in_array($args["iconPosition"], array("icon-box__icons--icons-left", "icon-box__icons--icons-std")))) {
            $e->add(t("The %s field has an invalid value.", t("Icon Position")));
        }
    ...
    {
        $al = AssetList::getInstance();
        $al->register('javascript', 'auto-js-vidal_themes_icon_box', 'blocks/vidal_themes_icon_box/auto.js', array(), 'modena');
        $this->requireAsset('javascript', 'auto-js-vidal_themes_icon_box');
        $this->edit();
    }
    ...
    public function fontAwesomeIcons($version = '4.2')
    {
        $icons = array(
             // 4.6
            'fa-american-sign-language-interpreting' => 'American Sign Language Interpreting', 'fa-asl-interpreting' => 'American Sign Language Interpreting', 'fa-assistive-listening-systems' => 'Assistive Listening Systems', 'fa-audio-description' => 'Audio description', 'fa-blind' => 'Blind', 'fa-braille' => 'Braille', 'fa-deaf' => 'Deaf', 'fa-deafness' => 'Deafness', 'fa-envira' => 'Envira', 'fa-fa'  => 'Font Awesome (alias)', 'fa-first-order'  => 'First Order', 'fa-font-awesome' => 'Font Awesome', 'fa-gitlab' => 'GitLab', 'fa-glide' => 'Glide', 'fa-glide-g' => 'Glide (g)', 'fa-google-plus-circle' => 'Google+ circle (alias)', 'fa-google-plus-official' => 'Google+', 'fa-hard-of-hearing' => 'Hard of hearing', 'fa-low-vision' => 'Low vision', 'fa-pied-piper' => 'Pied Piper', 'fa-question-circle-o' => 'Question circle o', 'fa-sign-language' => 'Sign language', 'fa-signing' => 'Signing', 'fa-snapchat' => 'SnapChat', 'fa-snapchat-ghost' => 'Snapchat ghost', 'fa-snapchat-square' => 'Snapchat square', 'fa-themeisle' => 'ThemeIsle', 'fa-universal-access' => 'Universall Access', 'fa-viadeo' => 'Viadeo', 'fa-viadeo-square' => 'Viadeo square', 'fa-volume-control-phone' => 'Volume control phone', 'fa-wheelchair-alt' => 'Wheelchair alt', 'fa-wpbeginner' => 'WPBeginner', 'fa-wpforms' => 'WPForms', 'fa-yoast' => 'Yoast',
            // 4.5
        ...
        if (version_compare($version, '4.6', '<')) {
            $unsets = array('fa-american-sign-language-interpreting', 'fa-asl-interpreting', 'fa-assistive-listening-systems', 'fa-audio-description', 'fa-blind', 'fa-braille', 'fa-deaf', 'fa-deafness', 'fa-envira', 'fa-fa', 'fa-first-order', 'fa-font-awesome', 'fa-gitlab', 'fa-glide', 'fa-glide-g', 'fa-google-plus-circle', 'fa-google-plus-official', 'fa-hard-of-hearing', 'fa-low-vision', 'fa-pied-piper', 'fa-question-circle-o', 'fa-sign-language', 'fa-signing', 'fa-snapchat', 'fa-snapchat-ghost', 'fa-snapchat-square', 'fa-themeisle', 'fa-universal-access', 'fa-viadeo', 'fa-viadeo-square', 'fa-volume-control-phone', 'fa-wheelchair-alt', 'fa-wpbeginner', 'fa-wpforms', 'fa-yoast');
            ...
            if (version_compare($version, '4.5', '<')) {
                $unsets = array('fa-bluetooth', 'fa-bluetooth-b', 'fa-codiepie', 'fa-credit-card-alt', 'fa-edge', 'fa-fort-awesome', 'fa-hashtag', 'fa-mixcloud', 'fa-modx', 'fa-pause-circle', 'fa-pause-circle-o', 'fa-percent', 'fa-product-hunt', 'fa-reddit-alien', 'fa-scribd', 'fa-shopping-bag', 'fa-shopping-basket', 'fa-stop-circle', 'fa-stop-circle-o', 'fa-usb' => 'USB');
                foreach ($unsets as $unset) {
                    if (isset($icons[$unset])) {
                        unset($icons[$unset]);
                    }
                ...
                if (version_compare($version, '4.4', '<')) {
                    ...
                    if (version_compare($version, '4.3', '<')) {
                        $unsets = array('fa-bed', 'fa-buysellads', 'fa-cart-arrow-down', 'fa-cart-plus', 'fa-connectdevelop', 'fa-dashcube', 'fa-diamond', 'fa-facebook-official', 'fa-forumbee', 'fa-heartbeat', 'fa-hotel', 'fa-leanpub', 'fa-mars', 'fa-mars-double', 'fa-mars-stroke', 'fa-mars-stroke-h', 'fa-mars-stroke-v', 'fa-medium', 'fa-mercury', 'fa-motorcycle', 'fa-neuter', 'fa-pinterest-p', 'fa-sellsy', 'fa-server', 'fa-ship', 'fa-shirtsinbulk', 'fa-simplybuilt', 'fa-skyatlas', 'fa-street-view', 'fa-subway', 'fa-train', 'fa-transgender', 'fa-transgender-alt', 'fa-user-plus', 'fa-user-secret', 'fa-user-times', 'fa-venus', 'fa-venus-double', 'fa-venus-mars', 'fa-viacoin', 'fa-whatsapp');
                        foreach ($unsets as $unset) {
                            if (isset($icons[$unset])) {
                                unset($icons[$unset]);
                            }
