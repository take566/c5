Concrete.event.bind('btVidalThemesQuoteSlider.testimonialSlider.edit.open', function (options, settings) {
    ...
    blockDesignerEditor('#' + ($(newField).find('textarea.ft-testimonialSlider-testimonialQuote').attr('id')).replace(/(:|\.|\[|\]|,|=|@)/g, "\\$1"));
    var ftImageTestimonialImage = $(newField).find('.ft-image-testimonialImage-file-selector');
            if ($(ftImageTestimonialImage).length > 0) {
                $(ftImageTestimonialImage).concreteFileSelector({'inputName': $(ftImageTestimonialImage).attr('data-file-selector-input-name'), 'filters': [], 'fID' : $(ftImageTestimonialImage).attr('data-file-selector-f-id') });
            }
            var pageSelectorTestimonialImage = $(newField).find('.ft-image-testimonialImage-page-selector');
            if ($(pageSelectorTestimonialImage).length > 0) {
                $(pageSelectorTestimonialImage).concretePageSelector({'inputName': $(pageSelectorTestimonialImage).attr('data-input-name'), 'cID' : $(pageSelectorTestimonialImage).attr('data-cID')});
            }
    };
        ...
        if (deleteIt === true) {
            ...
            $(sortableItem).slideUp(slideSpeed, function () {
                if(typeof CKEDITOR !== 'undefined'){
                    var blockDesignerEditorInstance = CKEDITOR.instances[$(sortableItem).find('textarea.ft-testimonialSlider-testimonialQuote').attr('id')];
