Concrete.event.bind('btVidalThemesTabs.addTabs.edit.open', function (options, settings) {
    ...
    //     $(newField).find('textarea.ft-addTabs-tabContent').redactor({
    ...
    var updateItem = function (newField) {
        $(newField).find(".launch-tooltip").tooltip({container: "#ccm-tooltip-holder"});
    blockDesignerEditor('#' + ($(newField).find('textarea.ft-addTabs-tabContent').attr('id')).replace(/(:|\.|\[|\]|,|=|@)/g, "\\$1"));
    };
        ...
        if (deleteIt === true) {
            ...
            $(sortableItem).slideUp(slideSpeed, function () {
                if(typeof CKEDITOR !== 'undefined'){
                    var blockDesignerEditorInstance = CKEDITOR.instances[$(sortableItem).find('textarea.ft-addTabs-tabContent').attr('id')];
