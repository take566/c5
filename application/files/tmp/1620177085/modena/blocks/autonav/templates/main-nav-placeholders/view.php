...
foreach ($navItems as $ni) {
    ...
    */
    ...
    if ($ni->hasSubmenu) {
        ...
        $classes[] = 'has-submenu has-submenu--mobile';
    }
...
if (count($navItems) > 0) {
    ...
    foreach ($navItems as $ni) {
        ...
        if($ni->level == '1'){
            echo '<li class="' . $ni->classes . ' nav-item">'; //opens a nav item
        } else {
            echo '<li class="' . $ni->classes . '">';
        }

        // Creates a dead link for the sub nav items so they cannot be clicked.

        if ($ni->hasSubmenu) {
            echo '<a href="#" target="' . $ni->target . '" class="' . $ni->classes . '">' . $ni->name . '</a>';
            echo '<ul class="is-submenu">'; //opens a dropdown sub-menu
        } else {
