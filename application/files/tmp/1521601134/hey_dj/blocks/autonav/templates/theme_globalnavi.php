...
<div class="navbar-header">
	...
	<?php
		$toppage_url = Core::make('helper/navigation')->getLinkToCollection(Page::getByID(1));
	?>
	<a class="navbar-brand img-top" href="<?php echo $toppage_url ?>">
		<?php
		$stack = Stack::getByName('title logo');
		$stack->display();
	...
	<ul class="nav navbar-nav my-navbar-nav navbar-right">
		<?php
		    foreach ($navItems as $ni) {
		    	...
		    	if ($ni->attrClass) {
		    		$ni->name = '<i class="fa fa-' . $ni->attrClass . ' fa-fw"></i>' . $ni->name;
		    	}

		    	if ($ni->hasSubmenu) {
		        	echo '<li class="dropdown">'; //opens a nav item
		        	echo '<a href="' . $ni->url . '" class="dropdown-toggle" data-toggle="dropdown" role="button" aria-expanded="false">' . $ni->name . '<span class="caret"></span></a>';
		    	} else {
