<?php namespace Concrete\Package\Modena\Block\VidalThemesVideoHeroUnit;
...
class Controller extends BlockController
{
    ...
    protected $btExportFileColumns = ['videoBackground', 'videoBackgroundBackup', 'posterImage'];
    protected $btExportPageColumns = ['buttonUrl'];
    protected $btTable = 'btVidalThemesVideoHeroUnit';
    ...
    public function getSearchableContent()
    {
        ...
        if (trim($this->externalLink_text) != "") {
            $content[] = $this->externalLink_text;
        } else {
            $content[] = $this->externalLink;
        }
    ...
    public function view()
    {
        ...
        $videoBackground_id = (int)$this->videoBackground;
        $this->videoBackground = false;
        if ($videoBackground_id > 0 && ($videoBackground_file = File::getByID($videoBackground_id)) && is_object($videoBackground_file)) {
            $fp = new Permissions($videoBackground_file);
            if ($fp->canViewFile()) {
                $urls = ['relative' => $videoBackground_file->getRelativePath()];
                if (($c = Page::getCurrentPage()) && $c instanceof Page) {
                    $urls['download'] = URL::to('/download_file', $videoBackground_id, $c->getCollectionID());
                }
                $videoBackground_file->urls = $urls;
                $this->videoBackground = $videoBackground_file;
            }
        }
        $this->set("videoBackground", $this->videoBackground);
        $videoBackgroundBackup_id = (int)$this->videoBackgroundBackup;
        $this->videoBackgroundBackup = false;
        if ($videoBackgroundBackup_id > 0 && ($videoBackgroundBackup_file = File::getByID($videoBackgroundBackup_id)) && is_object($videoBackgroundBackup_file)) {
            $fp = new Permissions($videoBackgroundBackup_file);
            if ($fp->canViewFile()) {
                $urls = ['relative' => $videoBackgroundBackup_file->getRelativePath()];
                if (($c = Page::getCurrentPage()) && $c instanceof Page) {
                    $urls['download'] = URL::to('/download_file', $videoBackgroundBackup_id, $c->getCollectionID());
                }
                $videoBackgroundBackup_file->urls = $urls;
                $this->videoBackgroundBackup = $videoBackgroundBackup_file;
            }
        }
        $this->set("videoBackgroundBackup", $this->videoBackgroundBackup);
        ...
        if ($this->posterImage && ($f = File::getByID($this->posterImage)) && is_object($f)) {
            $this->set("posterImage", $f);
        } else {
            $this->set("posterImage", false);
        }
    }
    ...
    public function save($args)
    {
        ...
        if (!isset($args["loopVideo"]) || trim($args["loopVideo"]) == "" || !in_array($args["loopVideo"], [0, 1])) {
            $args["loopVideo"] = '';
        }
        if (!isset($args["muteVideo"]) || trim($args["muteVideo"]) == "" || !in_array($args["muteVideo"], [0, 1])) {
            $args["muteVideo"] = '';
        }
        parent::save($args);
    }
    ...
    public function validate($args)
    {
        ...
        if (in_array("videoBackground", $this->btFieldsRequired) && (!isset($args["videoBackground"]) || trim($args["videoBackground"]) == "" || !is_object(File::getByID($args["videoBackground"])))) {
            $e->add(t("The %s field is required.", t("Video Background (MP4)")));
        }
        if (in_array("videoBackgroundBackup", $this->btFieldsRequired) && (!isset($args["videoBackgroundBackup"]) || trim($args["videoBackgroundBackup"]) == "" || !is_object(File::getByID($args["videoBackgroundBackup"])))) {
            $e->add(t("The %s field is required.", t("Video Background (WEBM)")));
        }
        ...
        if ((in_array("buttonAnimation", $this->btFieldsRequired) && (!isset($args["buttonAnimation"]) || trim($args["buttonAnimation"]) == "")) || (isset($args["buttonAnimation"]) && trim($args["buttonAnimation"]) != "" && !in_array($args["buttonAnimation"], ["none", "animate__bounce-in", "animate__bounce-in-down", "animate__bounce-in-right", "animate__bounce-in-up", "animate__bounce-in-left", "animate__fade-in-down-short", "animate__fade-in-up-short", "animate__fade-in-left-short", "animate__fade-in-right-short", "animate__fade-in-down", "animate__fade-in-up", "animate__fade-in-left", "animate__fade-in-right", "animate__fade-in", "animate__grow-in", "animate__shake", "animate__shake-up", "animate__rotate-in", "animate__rotate-in-up-left", "animate__rotate-in-up-right", "animate__rotate-in-down-right", "animate__rotate-in-down-left", "animate__roll-in", "animate__wiggle", "animate__swing", "animate__tada", "animate__wobble", "animate__pulse", "animate__light-speed-in-right", "animate__light-speed-in-left", "animate__flip", "animate__flip-in-x", "animate__flip-in-y"]))) {
        ...
        if ((in_array("headingAnimation", $this->btFieldsRequired) && (!isset($args["headingAnimation"]) || trim($args["headingAnimation"]) == "")) || (isset($args["headingAnimation"]) && trim($args["headingAnimation"]) != "" && !in_array($args["headingAnimation"], ["none", "animate__bounce-in", "animate__bounce-in-down", "animate__bounce-in-right", "animate__bounce-in-up", "animate__bounce-in-left", "animate__fade-in-down-short", "animate__fade-in-up-short", "animate__fade-in-left-short", "animate__fade-in-right-short", "animate__fade-in-down", "animate__fade-in-up", "animate__fade-in-left", "animate__fade-in-right", "animate__fade-in", "animate__grow-in", "animate__shake", "animate__shake-up", "animate__rotate-in", "animate__rotate-in-up-left", "animate__rotate-in-up-right", "animate__rotate-in-down-right", "animate__rotate-in-down-left", "animate__roll-in", "animate__wiggle", "animate__swing", "animate__tada", "animate__wobble", "animate__pulse", "animate__light-speed-in-right", "animate__light-speed-in-left", "animate__flip", "animate__flip-in-x", "animate__flip-in-y"]))) {
        ...
        if (in_array("heroUnitBottomMargin", $this->btFieldsRequired) && (trim($args["heroUnitBottomMargin"]) == "")) {
            $e->add(t("The %s field is required.", t("Hero Unit Bottom Margin")));
        }
        if (in_array("posterImage", $this->btFieldsRequired) && (trim($args["posterImage"]) == "" || !is_object(File::getByID($args["posterImage"])))) {
            $e->add(t("The %s field is required.", t("Poster Image")));
        }
        if (in_array("buttonUrl", $this->btFieldsRequired) && (trim($args["buttonUrl"]) == "" || $args["buttonUrl"] == "0" || (($page = Page::getByID($args["buttonUrl"])) && $page->error !== false))) {
            $e->add(t("The %s field is required.", t("Internal Link")));
        }
        ...
        if (in_array("loopVideo", $this->btFieldsRequired) && (trim($args["loopVideo"]) == "" || !in_array($args["loopVideo"], [0, 1]))) {
            $e->add(t("The %s field is required.", t("Loop Video")));
        }
        if (in_array("muteVideo", $this->btFieldsRequired) && (trim($args["muteVideo"]) == "" || !in_array($args["muteVideo"], [0, 1]))) {
            $e->add(t("The %s field is required.", t("Mute Video")));
        }
        return $e;
    }
